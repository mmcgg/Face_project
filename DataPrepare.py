...
class ImagePrepare(object):
    ...
    def image_data_prepare(self):
        ...
        for img_name in self.imgs_name_list:
            ...
            if img.shape == (250, 250, 3):
                ...
                five_landmarks = get_five_points_landmarks(self.path+'/'+img_name)
                self.landmarks.append(five_landmarks)
                continue
            face_location = detector(img, 1)[0]
            top = face_location.top()
            bottom = face_location.bottom()
            left = face_location.left()
            right = face_location.right()
            img = img[top-15: bottom+15, left-15: right+15]
    ...
    def get_imgs_features(self, imgs_alignment):
        input_images = Variable(torch.from_numpy(imgs_alignment).float(), volatile=True).cuda()
        output_features = net(input_images)
        output_features = output_features.data.cpu().numpy()
...
f1, f2 = output_images_features[0], output_images_features[3]
